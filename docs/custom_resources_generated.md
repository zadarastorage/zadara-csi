...
| `spec.isDefault`            | boolean | Default VSCStorageClass will be used when VSCStorageClassName is           |          |
...
| `spec.volumeFlags.extra`    | object  | Additional Volume flags.                                                   |          |
...
| `status.state`              | string  | Status of the VSCStorageClass                                                                       | Required. Allowed values: `"Creating"`, `"Ready"`, `"Failed"`, `"Deleting"` |
|                             |         | 	`Creating`: Exists in DB.                                                                           |                                                                             |
|                             |         | 	`Ready`:    Ready for volume provisioning.                                                          |                                                                             |
|                             |         | 	`Failed`:   VUnhealthy, typically because of failed VPSA which are members of this VSCStorageClass. |                                                                             |
...
| `spec.VSCStorageClassName` | string | Name of the VSCStorageClass Custom Resource,           | Required |
...
| `status.state`                   | string  | Status of the VPSA                                                                   | Required. Allowed values: `"Configuring"`, `"Ready"`, `"Incompatible"`, `"Unreachable"`, `"Failed"`, `"Deleting"` |
|                                  |         | 	`Configuring`:  Exists in DB, validation and reconfiguring in progress.              |                                                                                                                   |
|                                  |         | 	`Ready`:        Ready for volume provisioning                                        |                                                                                                                   |
|                                  |         | 	`Incompatible`: Does not fit VSC requirements or not compatible with VSCStorageClass |                                                                                                                   |
|                                  |         | 	`Unreachable`:  Unreachable - state unknown (network, credentials or TLS issues).    |                                                                                                                   |
|                                  |         | 	`Failed`:       Unhealthy (Pools, Drives, Volumes not healthy)                       |                                                                                                                   |
|                                  |         | 	`Deleting`:     VPSA is removed from VSCStorageClass                                 |                                                                                                                   |
...
| `spec.VSCStorageClassName` | string  | Name of the VSCStorageClass Custom Resource (i.e, a group of VPSAs), | Required                                     |
...
| `spec.flags`               | object  | Additional flags used when creating a new Volume.                    |                                              |
...
| `spec.flags.extra`         | object  | Additional Volume flags.                                             |                                              |
|                            |         | See "Create Volume" in https://vpsa-api.zadarastorage.com/#volumes   |                                              |
...
| `status.state`                 | string  | Status of the Snapshot                                                                      | Required |
...
It uses the same schema as Volume, but represents a Volume that is not managed by VSC (Volume Service Controller).
Deleting ExternalVolume will not delete the underlying VPSA Volume.
```shell script
...
| `status.NAS`               | object  |                                                                                                                         |          |
| `status.VPSAServerID`      | string  | Internal Server ID on the VPSA, e.g, srv-00000001.                                                                      |          |
|                            |         | Note, that Servers are shared between VolumeAttachments on the same Node.                                               |          |
| `status.state`             | string  | Status of the VolumeAttachment                                                                                          | Required |
|                            |         | 	`Creating`:          VolumeAttachment exists in DB.                                                                     |          |
|                            |         | 	`Attaching`:         Server exists on Volume's VPSA, VPSAServerID is set.                                               |          |
|                            |         | 	`ISCSILoginPending`: Volume is attached to Server on VPSA, waiting for Node to establish iSCSI connection (Block only). |          |
|                            |         | 	`Ready`:             Volume is attached and ready to mount.                                                             |          |
|                            |         | 	`Detaching`:         Detach requested.                                                                                  |          |
|                            |         | 	`Deleting`:          Volume detached from VPSA Server, try to delete VPSA Server.                                       |          |
|                            |         | 	`Failed`:            VolumeAttachment is in bad state.                                                                  |          |
| `status.Block`             | object  |                                                                                                                         |          |
...
| `spec.volumeID`    | string | Name of the Volume Custom Resource,                | Required |
...
| `status.size`             |        | Size is the size of source Volume at the time of creating Snapshot,          |                   |
|                           |        | and it also determines the size of cloned Volume.                            |                   |
| `status.state`            | string | Status of the Snapshot                                                       |                   |
|                           |        | 	`Creating`:        Snapshot Exists in DB.                                    |                   |
|                           |        | 	`TakingSnapshot`:  Snapshot exists on VPSA, waiting for it to be ready.      |                   |
|                           |        | 	`Ready`:           Snapshot is ready to use.                                 |                   |
|                           |        | 	`Deleting`:        Snapshot in being deleted, delete on VPSA is in progress. |                   |
|                           |        | 	`Failed`:          Snapshot is in bad state.                                 |                   |
